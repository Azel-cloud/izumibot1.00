...
const vcard = 'BEGIN:VCARD\n' // ANAK ANJING MAU NGAPAIN ASU!?
            ...
            + 'FN:Izumi Botz\n' // NAMA OWNER BOT
            + 'ORG:Creator fadhil graphy;\n' // KASIH CREDITS GUA FADHIL GRAPHY SU!!!
            + 'TEL;type=CELL;type=VOICE;waid=6285787487696:+62 857-8748-7696\n' // GANTI NOMOR LU
            + 'END:VCARD'
prefix = '!'
blocked = []
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `SAYONARA... @${num.split('@')[0]}👋* \n_Hati-Hati Dijalan, Kalo Balik Nitip Gorengan Ya Awas aja kaga \nlop yu dari izumi botz_`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			case 'owner':
                 ...
                 client.sendMessage(from, 'itu owner izumi yaa jangan spam ><',MessageType.text, { quoted: mek} )
                 break
                 case 'fitnah':	
				case 'fake':          
                    ...
                    break
                case 'fakereplay':
                   client.reply(from, 'ange mas', 'mending lari', "0857-87487696")
                   break
                case 'but':
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
                ...
                case 'report':
                    ...
                    client.sendMessage('6285787487696@s.whatsapp.net', options, text, {quoted: mek})
               ...
               case 'infogempa':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ Ini Broadcastl ]\n\n${body.slice(4)}`})
						}
						reply('Suksess broadcast')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ *IZUMIBOTZ BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
